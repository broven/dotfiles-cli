...
Below is an example of `mappings.yaml`.  You can use `~` to represent a home directory.  As key, you can specify a name of file or directory in your dotfiles repository.  They will be linked to the corresponding values as symbolic links.  At least one namespace from `link`, `partial_link`, `npm`, or `homebrew` is required.
...
You can also configure package manager namespaces.  `dotfiles link` will install/update configured packages.  If `npm` or `brew` command is not found, it will skip that namespace with a notification.
...
As a shorthand, `homebrew` can be a plain list.  It is treated as `formula` packages.
